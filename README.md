# 发源地/发源链开源云采集引擎节点
发源地/发源链云采集引擎是由发源链团队研发的一套开源分布式/去中心化云采集机器人节点引擎，致力于让用户快捷挖掘大数据矿山背后的价值！
发源地/发源链云采集引擎集数据采集、清洗、去重、加工于一体的互联网WEB/APP数据采集引擎，支持本地化私有部署，可以打破数据孤岛，快速搭建属于自己的大数据云采集/爬虫系统。用户可以低成本、高效率完成网页中文本，图片等资源信息的采集，并进行过滤加工，挖掘出精准所需的数据，让数据以结构化的文件包、采集规则算法或API接口方式输出，同时可以选择发布到发源链数据交易平台DApp进行交易，或导出为Excel、CSV、SQL等格式的文件保存在本地。
...
发源地云采集引擎是国内领先的分布式大数据云采集工具化引擎，支持海量数据节点的全自动化接入，去重、清洗、脱敏及结构化存储。
